...
const MemberTabsList = ({ totalMembers, totalInvitation,totalTeam }) => {
    return (
        <TabsList className="px-0 space-x-6">
            ...
            <TabsTrigger className="px-0" value="Invitations">
                Invitations
                <span className="mx-1 inline-flex items-center justify-center w-5 h-5 rounded-full text-white bg-red text-font-12 font-normal leading-4 text-center">
                    {totalInvitation}
                </span>
            </TabsTrigger>
            {/* <TabsTrigger className="px-0" value="Request">
                Request
                <span className="mx-1 inline-flex items-center justify-center w-5 h-5 rounded-full text-white bg-red text-font-12 font-normal leading-4 text-center">
                    3
                </span>
...
export default function MembersSettings() {
    ...
    const [totalInvitationMember, setTotalInvitationMember] = useState(0);
    ...
    const { getUsersList,totalInvitation } = useUsers();
    ...
    useEffect(()=>{
        setTotalInvitationMember(totalInvitation)
    },[totalInvitation])
                ...
                <div className="h-full overflow-y-auto w-full relative">
                    <div className="mx-auto max-w-[950px]">
                        ...
                        <Tabs defaultValue="Members" className="w-full" onValueChange={(newValue) => { setSelectedTab(newValue) }}>
                            <MemberTabsList totalMembers={totalMembers} totalInvitation={totalInvitation} totalTeam={totalTeam} />
                            ...
                            <TabsContent value="Invitations" className="p-0 max-w-[calc(100vw-25px)] overflow-x-auto">
                                <Invitation handleOpen={handleOpen} isOpen={isOpen} selectedTab={selectedTab} setTotalInvitation={setTotalInvitationMember} totalInvitation={totalInvitationMember} />
                            </TabsContent>
                        </Tabs>
                    </div>
                </div>
...
const Member = ({ selectedTab, membersOptions, setTotalMembers, totalMembers }) => {
    ...
    const handleToggleUser = async ({userIds=[],enableAll=false,disableAll=false}) => {
        ...
        if (enableAll || disableAll) {
            ...
            userIds = users.reduce((acc, currUser) => {
                if (targetRoles.includes(currUser.roleCode)) {
    ...
    let columns = [
        ...
        },
        {
            ...
            //     menuPlacement="auto"
            //     id="membersDes"
            //     className="react-select-container react-select-border-light bg-white react-select-sm w-[150px]"
        ...
        // },
        {
            header: () => (
                ...
                <span className="">Private Brain</span> {/* Adjust the margin-right */}
                <DropdownMenu>
                    <DropdownMenuTrigger
...
const Invitation = ({ selectedTab, isOpen, handleOpen, setTotalInvitation, totalInvitation }) => {
    ...
    const renderStatus = (status) => {
        switch (status?.toLowerCase()) {
            ...
            case 'pending':
                return <span className="text-font-14 text-b5">Pending</span>;
            case 'expired':
