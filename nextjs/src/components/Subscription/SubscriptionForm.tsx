...
const SubscriptionFormWrapper = ({ isUpdate = false, closeModal, planqty, planPriceId }:any) => {
    return (
        <Elements stripe={stripePromise}>
            <SubscriptionForm isUpdate={isUpdate} closeModal={closeModal} planqty={planqty} planPriceId={planPriceId} />
        </Elements>
    );
};

const SubscriptionForm = ({ isUpdate = false, closeModal, planqty, planPriceId}) => {
    ...
    useEffect(() => {
        const priceId = planPriceId || (isIndiaByTimezone() ? STRIPE_SUBSCRIPTION_PRICE_ID_IND : STRIPE_SUBSCRIPTION_PRICE_ID);
    ...
    const handleSubmit = async (event) => {
        ...
        if (isUpdate) {
            if(subscriptionData.quantity == quantity && couponCode == null){
                Toast(`No need to update Subscription due to same quantity and no coupon code`,"error")
                return;
            }
            const payload = {
                quantity: quantity,
            ...
            result = await handleSubscription(payload);
            ...
            if (result?.requiresAction === true) {
                ...
                if (paymentIntent.status === 'succeeded') {
                    ...
                    result={...result,...payload,price_id:priceId}
