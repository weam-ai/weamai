...
const ScrollToBottomButton = React.memo(({ contentRef }) => {
  ...
  return (
    <>
      {showButton && <button onClick={scrollToBottom} className='w-[36px] h-[36px] bg-white border fixed bottom-8 right-8 z-10 border-b10 rounded-full transition ease-in-out duration-150 flex items-center justify-center [&>svg]:h-[19] [&>svg]:w-[15px] [&>svg]:object-contain [&>svg]:fill-b2 hover:bg-b11'>
