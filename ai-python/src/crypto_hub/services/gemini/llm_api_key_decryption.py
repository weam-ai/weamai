...
class LLMAPIKeyDecryptionHandler:
    ...
    def initialization(self, api_key_id, collection_name,**kwargs):
        try:
            ...
            # self.apikey = self.api_key_selector_service.sync_get_best_api_key(provider=self.bot_data.get('code', 'GEMINI'), model=self.model_name, functionality=self.functionality,company_id=self.companyRedis_id)
            if not security_key:
                    logger.error(
