from src.chatflow_langchain.service.import_chat.import_data import ImportData
...
class ImportChatController:
    ...
    async def upload_file(self,file,file_content):
        try:
            ...
            upload_stream_to_s3.apply_async(args=[self.file_content,s3_bucket,self.file_uri])
    ...
    async def get_import_data(self,current_user):
        try:
            ...
            self.import_data_service = ImportData(import_id=self.import_id, zip_file_bytes=self.file_content,config=self.import_input,brain_id=self.import_input['brain_id'])
    ...
    def pipeline_initlization(self):
        if self.import_data_service.html_files:
            logger.info("HTML files found, using OpenAI processor.",
                extra={"tags": {"method": "ImportChatController.pipeline_initialization"}})
            self.service=self.controller["OPENAI"]()  
        else:
            logger.info("No HTML files found, using Anthropic processor.",
                            extra={"tags": {"method": "ImportChatController.pipeline_initialization"}})
            self.service=self.controller["ANTHROPIC"]()
    
    async def process_conversations(self):
        try:
            await self.service.initlization(import_input=self.import_input,user_data=self.import_data_service.user_data,team_dict=self.import_data_service.team_dict,current_user=self.current_user)
