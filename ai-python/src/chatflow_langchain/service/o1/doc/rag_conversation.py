...
from src.chatflow_langchain.service.o1.doc.chat_prompt_factory import create_chat_prompt_doc
...
from src.chatflow_langchain.service.o1.doc.config import DocConfig
...
from src.chatflow_langchain.service.o1.doc.utils import extract_error_message
...
class O1StreamingDocumentedChatService(AbstractConversationService):
    def initialize_llm(self, api_key_id: str = None, companymodel: str = None,thread_id:str=None,thread_model:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.initialize_llm"}}
            )
    ...
    def initialize_repository(self, chat_session_id: str = None, collection_name: str = None,regenerated_flag:bool=False,thread_id:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.initialize_repository"}}
            )
        ...
        **kwargs
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.load_vector_store"}}
            )
    ...
    def prompt_attach(self, additional_prompt_id: str = None, collection_name: str = None):
        ...
        try:
            if additional_prompt_id:
                ...
                if resource_key is not None and resource_value is not None:
                    ...
                    logger.info("Successfully attached additional prompt", extra={
                        "tags": {"method": "O1StreamingDocumentedChatService.prompt_attach"},
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.prompt_attach"}}
            )
    ...
    def create_conversation(self,tag: str,input_text:str,**kwargs):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.create_conversation"}}
            )
    ...
    async def stream_run_conversation(self, thread_id: str, collection_name: str,regenerated_flag:bool=False,msgCredit:float=0,is_paid_user:bool=False,**kwargs) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            async with  \
                    get_custom_openai_callback(llm_apikey_decrypt_service.model_name, cost=cost, thread_id=thread_id, collection_name=collection_name,encrypted_key=self.encrypted_key,companyRedis_id=self.companyRedis_id,**kwargs) as cb, \
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    ...
                    extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.NotFoundError"}})
            else:
                logger.error(
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.RateLimitError"}})
            else:
                logger.error(
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.APIStatusError"}})
                thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.LengthFinishReasonError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.ContentFilterFinishReasonError"}})
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.APITimeoutError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.APIConnectionError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": str(e), "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in OPENAI_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.Exception_Try"}})
                    else:
                        logger.error(
                    ...
                    logger.error(
                        ...
                        extra={"tags": {"method": "O1StreamingDocumentedChatService.stream_run_conversation.Exception_Except"}})
                    thread_repo.initialization(thread_id, collection_name)
                    thread_repo.add_message_openai("common_response")
                    content = OPENAI_MESSAGES_CONFIG.get("common_response")
    ...
    def cleanup(self):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1StreamingDocumentedChatService.cleanup"}}
            )