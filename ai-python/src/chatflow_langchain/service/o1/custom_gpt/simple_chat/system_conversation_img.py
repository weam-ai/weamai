...
from src.chatflow_langchain.service.o1.custom_gpt.simple_chat.chat_prompt_factory import UserCustomGPTPrompt
...
from src.chatflow_langchain.service.o1.custom_gpt.config import CustomGptChatConfig,GetLLMkey,DEFAULTMODEL
...
from src.chatflow_langchain.service.o1.custom_gpt.simple_chat.utils import extract_error_message
from src.chatflow_langchain.service.o1.custom_gpt.simple_chat.tools import image_generate
...
class O1CustomGPTStreamingSimpleChatServiceImg(AbstractConversationService):
    def Initilization_custom_gpt(self,custom_gpt_id:str=None,customgptmodel:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.Initilization_custom_gpt"}}
            )
    ...
    def initialize_repository(self, chat_session_id: str = None, collection_name: str = None,regenerated_flag:bool=False,thread_id:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.initialize_repository"}}
            )
    ...
    def initialize_memory(self):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.initialize_memory"}}
            )
    ...
    def prompt_attach(self, additional_prompt_id: str = None, collection_name: str = None):
        ...
        try:
            if additional_prompt_id:
                ...
                if resource_key is not None and resource_value is not None:
                    ...
                    logger.info("Successfully attached additional prompt", extra={
                        "tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.prompt_attach"},
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.prompt_attach"}}
            )
    ...
    def create_conversation(self,input_text:str,**kwargs):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.create_conversation"}}
            )
    ...
    async def tool_calls_run(self, thread_id: str, collection_name: str,regenerated_flag=False,msgCredit=0,is_paid_user=False,**kwargs) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            for tool_call in ai_msg.tool_calls:
                ...
                "tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.tool_calls_run"}
            }) 
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    ...
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.tool_calls_run.NotFoundError"}})
            else:
                logger.error(
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.tool_calls_run.RateLimitError"}})
            else:
                logger.error(
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.tool_calls_run.APIStatusError"}})
                thread_repo.initialization(thread_id, collection_name)
    ...
    async def simple_run_conversation_v1(self) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            async with async_streaming_handler() as async_handler, \
                    get_custom_openai_callback(llm_apikey_decrypt_service.model_name, cost=cost, thread_id=self.thread_id, collection_name=self.collection_name,encrypted_key=self.encrypted_key,companyRedis_id=self.companyRedis_id,**self.kwargs) as cb, \
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.NotFoundError"}})
            else:
                logger.error(
                    f"🚨 Model Not Found Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.NotFoundError"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.RateLimitError"}})
            else:
                logger.error(
                    f"🚨 OpenAI Rate limit exceeded: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.RateLimitError"}})
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.APIStatusError"}})
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.LengthFinishReasonError"}})
            thread_repo.initialization(self.thread_id, self.collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.ContentFilterFinishReasonError"}})
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.APITimeoutError"}}
            )
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.APIConnectionError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            try:
                ...
                if error_code not in OPENAI_MESSAGES_CONFIG:
                    logger.warning(
                        f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                        extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.Exception_Try"}})
                else:
                    logger.error(
                ...
                logger.error(
                    ...
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.stream_run_conversation_Exception_Except"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
    ...
    async def web_search_preview(self, **kwargs):
        try:
            ...
            logger.info(f"Tool 'web_search_preview' called", extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview"}})
      
            thread_id=self.thread_id
            thread_model=self.collection_name
            self.astream_llm = ChatOpenAI(temperature=1.0,
                    api_key=llm_apikey_decrypt_service.decrypt(),
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.NotFoundError"}})
            else:
                logger.error(
                    f"🚨 Model Not Found Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.NotFoundError"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.RateLimitError"}})
            else:
                logger.error(
                    f"🚨 OpenAI Rate limit exceeded: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.RateLimitError"}})
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.APIStatusError"}})
                thread_repo.initialization(thread_id, thread_model)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            logger.error(
                f"OpenAI Length Finish Reason Error: {e}",
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.LengthFinishReasonError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Content Filter Error: {e}",
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.ContentFilterFinishReasonError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Timeout Error: {e}",
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.APITimeoutError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("request_time_out")
            content = OPENAI_MESSAGES_CONFIG.get("request_time_out", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Connection Error: {e}",
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.APIConnectionError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("connection_error")
            content = OPENAI_MESSAGES_CONFIG.get("connection_error", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": str(e), "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in OPENAI_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.Exception_Try"}})
                    else:
                        logger.error(
                            f"🚨 Failed to stream run conversation: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.Exception_Try"}})
                    ...
                    logger.error(
                        f"🚨 Failed to stream run conversation: {e}",
                        extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.web_search_preview.Exception_Except"}})
                    thread_repo.initialization(thread_id, thread_model)
                    thread_repo.add_message_openai("common_response")
                    content = OPENAI_MESSAGES_CONFIG.get("common_response")
                    yield json.dumps({"status": status.HTTP_400_BAD_REQUEST, "message": DEV_MESSAGES_CONFIG.get("dev_message"), "data": content}), status.HTTP_400_BAD_REQUEST
    ...
    async def website_analysis(self,**kwargs):
        try:
            logger.info(f"Tool 'website_analysis' called", extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.NotFoundError"}})
            else:
                logger.error(
                    f"🚨 Model Not Found Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.NotFoundError"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.RateLimitError"}})
            else:
                logger.error(
                    f"🚨 OpenAI Rate limit exceeded: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.RateLimitError"}})
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.APIStatusError"}})
                thread_repo.initialization(thread_id, thread_model)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            logger.error(
                f"OpenAI Length Finish Reason Error: {e}",
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.LengthFinishReasonError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Content Filter Error: {e}",
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.ContentFilterFinishReasonError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Timeout Error: {e}",
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.APITimeoutError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("request_time_out")
            content = OPENAI_MESSAGES_CONFIG.get("request_time_out", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Connection Error: {e}",
                extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.APIConnectionError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("connection_error")
            content = OPENAI_MESSAGES_CONFIG.get("connection_error", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": str(e), "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in OPENAI_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.Exception_Try"}})
                    else:
                        logger.error(
                            f"🚨 Failed to stream run conversation: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.Exception_Try"}})
                    ...
                    logger.error(
                        f"🚨 Failed to stream run conversation: {e}",
                        extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.website_analysis.Exception_Except"}})
                    thread_repo.initialization(thread_id, thread_model)
                    thread_repo.add_message_openai("common_response")
                    content = OPENAI_MESSAGES_CONFIG.get("common_response")
                    yield json.dumps({"status": status.HTTP_400_BAD_REQUEST, "message": DEV_MESSAGES_CONFIG.get("dev_message"), "data": content}), status.HTTP_400_BAD_REQUEST
        ...
        try:
            ...
            if cleaned_up:
                logger.info(
                    ...
                    extra={"tags": {"method": "O1CustomGPTStreamingSimpleChatServiceImg.cleanup"}}
                )
