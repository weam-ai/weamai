    ...
    try:
        ...
        if error_code not in OPENAI_MESSAGES_CONFIG:
            logger.warning(
                ...
                extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.NotFoundError"}})
        else:
            logger.error(
        ...
        if error_code not in OPENAI_MESSAGES_CONFIG:
            logger.warning(
                f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.RateLimitError"}})
        else:
            logger.error(
        ...
        if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
            logger.warning(
                f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.APIStatusError"}})
            thread_repo.initialization(thread_id, thread_model)
        ...
        logger.error(
            ...
            extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.LengthFinishReasonError"}})
        thread_repo.initialization(thread_id, thread_model)
        ...
        logger.error(
            ...
            extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.ContentFilterFinishReasonError"}})
        thread_repo.initialization(thread_id, thread_model)
        thread_repo.add_message_openai("content_filter_issue")
        content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
        yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
        ...
        logger.error(
            ...
            extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.APITimeoutError"}})
        thread_repo.initialization(thread_id, thread_model)
        ...
        logger.error(
            ...
            extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.APIConnectionError"}})
        thread_repo.initialization(thread_id, thread_model)
        ...
        yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": str(e), "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            try:
                ...
                if error_code not in OPENAI_MESSAGES_CONFIG:
                    logger.warning(
                        f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                        extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.Exception_Try"}})
                else:
                    logger.error(
                ...
                logger.error(
                    ...
                    extra={"tags": {"method": "O1CustomGPTStreamingDocChatServiceImg.image_generate.Exception_Except"}})
                thread_repo.initialization(thread_id, thread_model)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
