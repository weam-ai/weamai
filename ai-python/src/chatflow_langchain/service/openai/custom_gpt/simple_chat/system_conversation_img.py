...
from src.chatflow_langchain.service.openai.custom_gpt.config import ToolChatConfig
...
from src.chatflow_langchain.service.openai.custom_gpt.simple_chat.chat_prompt_factory import UserCustomGPTPrompt
...
from src.chatflow_langchain.service.openai.custom_gpt.config import CustomGptChatConfig,GetLLMkey,DEFAULTMODEL
...
from src.chatflow_langchain.service.openai.custom_gpt.simple_chat.utils import extract_error_message
...
from src.chatflow_langchain.service.config.model_config_openai import OPENAIMODEL,WebSearchConfig
import re
...
class OpenAICustomGPTStreamingSimpleChatServiceImg(AbstractConversationService):
    def Initilization_custom_gpt(self,custom_gpt_id:str=None,customgptmodel:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.Initilization_custom_gpt"}}
            )
    ...
    def initialize_repository(self, chat_session_id: str = None, collection_name: str = None,regenerated_flag:bool=False,thread_id:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.initialize_repository"}}
            )
    ...
    def initialize_llm(self, company_id:str=None,companymodel: str = None,**kwargs):
        ...
        try:
            ...
            self.orginal_model=llm_apikey_decrypt_service.model_name
            ...
            if self.orginal_model in WebSearchConfig.MODEL_LIST:
                self.tools = [self.simple_run_conversation,self.wrapper_web_search_preview]
            else:
            ...
            self.llm_with_tools = self.llm_non_stream.bind_tools(self.tools, tool_choice='any',strict=False)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.initialize_llm"}}
            )
    ...
    def initialize_memory(self):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.initialize_memory"}}
            )
    ...
    def prompt_attach(self, additional_prompt_id: str = None, collection_name: str = None):
        ...
        try:
            if additional_prompt_id:
                ...
                if resource_key is not None and resource_value is not None:
                    ...
                    logger.info("Successfully attached additional prompt", extra={
                        "tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.prompt_attach"},
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.prompt_attach"}}
            )
    ...
    def create_conversation(self,input_text:str,**kwargs):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.create_conversation"}}
            )
    ...
    async def tool_calls_run(self, thread_id: str, collection_name: str,regenerated_flag=False, msgCredit=0,is_paid_user=False,**kwargs) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            with get_openai_callback() as cb:
                ...
                tool_history.append(HumanMessage(self.inputs['input']+' '.join(self.result)))
            ...
            for tool_call in ai_msg.tool_calls:
                ...
                "tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.tool_calls_run"}
            }) 
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    ...
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.tool_calls_run.NotFoundError"}})
            else:
                logger.error(
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.tool_calls_run.RateLimitError"}})
            else:
                logger.error(
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.tool_calls_run.APIStatusError"}})
                thread_repo.initialization(thread_id, collection_name)
    ...
    async def simple_run_conversation(self,**kwargs):
        pass

    @tool(description=ToolServiceDescription.WEB_SEARCH_PREVIEW)
    async def wrapper_web_search_preview(self, **kwargs):
        pass
    ...
    async def web_search_preview(self, **kwargs):
        '''
        This tool performs a contextual web search or generates a content preview based on user input for real time data,
        ...
        try:
            ...
            logger.info(f"Tool 'web_search_preview' called", extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview"}})
            ...
            web_query_input = {"input": self.original_query}  # Single query
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.NotFoundError"}})
            else:
                logger.error(
                    f"🚨 Model Not Found Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.NotFoundError"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.RateLimitError"}})
            else:
                logger.error(
                    f"🚨 OpenAI Rate limit exceeded: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.RateLimitError"}})
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.APIStatusError"}})
                thread_repo.initialization(thread_id, thread_model)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.LengthFinishReasonError"}})
            thread_repo.initialization(thread_id, thread_model)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.ContentFilterFinishReasonError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.APITimeoutError"}})
            thread_repo.initialization(thread_id, thread_model)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.APIConnectionError"}})
            thread_repo.initialization(thread_id, thread_model)
            ...
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": str(e), "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in OPENAI_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.Exception_Try"}})
                    else:
                        logger.error(
                    ...
                    logger.error(
                        ...
                        extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.web_search_preview.Exception_Except"}})
                    thread_repo.initialization(thread_id, thread_model)
                    thread_repo.add_message_openai("common_response")
                    content = OPENAI_MESSAGES_CONFIG.get("common_response")
    ...
    async def website_analysis(self,**kwargs):
        try:
            logger.info(f"Tool 'website_analysis' called", extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.NotFoundError"}})
            else:
                logger.error(
                    f"🚨 Model Not Found Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.NotFoundError"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.RateLimitError"}})
            else:
                logger.error(
                    f"🚨 OpenAI Rate limit exceeded: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.RateLimitError"}})
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.APIStatusError"}})
                thread_repo.initialization(thread_id, thread_model)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            logger.error(
                f"OpenAI Length Finish Reason Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.LengthFinishReasonError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Content Filter Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.ContentFilterFinishReasonError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Timeout Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.APITimeoutError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("request_time_out")
            content = OPENAI_MESSAGES_CONFIG.get("request_time_out", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Connection Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.APIConnectionError"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_openai("connection_error")
            content = OPENAI_MESSAGES_CONFIG.get("connection_error", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": str(e), "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in OPENAI_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.Exception_Try"}})
                    else:
                        logger.error(
                            f"🚨 Failed to stream run conversation: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.Exception_Try"}})
                    ...
                    logger.error(
                        f"🚨 Failed to stream run conversation: {e}",
                        extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.website_analysis.Exception_Except"}})
                    thread_repo.initialization(thread_id, thread_model)
                    thread_repo.add_message_openai("common_response")
                    content = OPENAI_MESSAGES_CONFIG.get("common_response")
                    yield json.dumps({"status": status.HTTP_400_BAD_REQUEST, "message": DEV_MESSAGES_CONFIG.get("dev_message"), "data": content}), status.HTTP_400_BAD_REQUEST
    ...
    async def simple_run_conversation_v1(self) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            async with async_streaming_handler() as async_handler, \
                    get_custom_openai_callback(self.model_name, cost=cost, thread_id=self.thread_id, collection_name=self.collection_name,encrypted_key=self.encrypted_key,companyRedis_id=self.companyRedis_id,**self.kwargs) as cb, \
                    get_mongodb_callback_handler(thread_id=self.thread_id, chat_history=self.chat_repository_history, memory=self.memory,collection_name=self.collection_name,regenerated_flag=self.regenerated_flag,msgCredit=self.msgCredit,is_paid_user=self.is_paid_user,encrypted_key=self.encrypted_key,companyRedis_id=self.companyRedis_id) as mongo_handler:
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.NotFoundError"}})
            else:
                logger.error(
                    f"🚨 Model Not Found Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.NotFoundError"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.RateLimitError"}})
            else:
                logger.error(
                    f"🚨 OpenAI Rate limit exceeded: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.RateLimitError"}})
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.APIStatusError"}})
            ...
            logger.error(
                f"OpenAI Length Finish Reason Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.LengthFinishReasonError"}})
            thread_repo.initialization(self.thread_id, self.collection_name)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Content Filter Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.ContentFilterFinishReasonError"}})
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"OpenAI Timeout Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.APITimeoutError"}}
            )
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("request_time_out")
            content = OPENAI_MESSAGES_CONFIG.get("request_time_out", OPENAI_MESSAGES_CONFIG.get("common_response"))
            ...
            logger.error(
                f"OpenAI Connection Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.APIConnectionError"}})
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("connection_error")
            content = OPENAI_MESSAGES_CONFIG.get("connection_error", OPENAI_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": str(e), "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            try:
                ...
                if error_code not in OPENAI_MESSAGES_CONFIG:
                    logger.warning(
                        f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                        extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.Exception_Try"}})
                else:
                    logger.error(
                        f"🚨 Failed to stream run conversation: {error_code}, Message: {error_content}",
                        extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation.Exception_Try"}})
                ...
                logger.error(
                    f"🚨 Failed to stream run conversation: {e}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.stream_run_conversation_Exception_Except"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
                yield json.dumps({"status": status.HTTP_400_BAD_REQUEST, "message": DEV_MESSAGES_CONFIG.get("dev_message"), "data": content}), status.HTTP_400_BAD_REQUEST
        ...
        try:
            ...
            if cleaned_up:
                logger.info(
                    ...
                    extra={"tags": {"method": "OpenAICustomGPTStreamingSimpleChatServiceImg.cleanup"}}
                )
