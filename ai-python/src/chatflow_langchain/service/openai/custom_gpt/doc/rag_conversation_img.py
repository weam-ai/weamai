...
from src.chatflow_langchain.service.openai.custom_gpt.config import CustomGptDocConfig,GetLLMkey,DEFAULTMODEL
...
from src.chatflow_langchain.service.openai.custom_gpt.doc.utils import extract_error_message
...
from src.chatflow_langchain.service.config.model_config_openai import DefaultGPTTextModelRepository, OPENAIMODEL,WebSearchConfig, Functionality
    ...
    def Initilization_custom_gpt(self,custom_gpt_id:str=None,customgptmodel:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.Initilization_custom_gpt"}}
            )
    ...
    def initialize_repository(self, chat_session_id: str = None, collection_name: str = None,regenerated_flag:bool=False,thread_id:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.initialize_repository"}}
            )
    ...
    def initialize_memory(self):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.initialize_memory"}}
            )
        ...
        **kwargs
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.load_vector_store"}}
            )
    ...
    def prompt_attach(self, additional_prompt_id: str = None, collection_name: str = None):
        ...
        try:
            if additional_prompt_id:
                ...
                if resource_key is not None and resource_value is not None:
                    ...
                    logger.info("Successfully attached additional prompt", extra={
                        "tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.prompt_attach"},
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.prompt_attach"}}
            )
    ...
    def create_conversation(self,input_text:str,**kwargs):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.create_conversation"}}
            )
    ...
    async def rag_run_conversation(self) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.NotFoundError"}})
            else:
                logger.error(
                    f"🚨 Model Not Found Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.NotFoundError"}})
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.RateLimitError"}})
            else:
                logger.error(
                    f"🚨 OpenAI Rate limit exceeded: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.RateLimitError"}})
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.APIStatusError"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            logger.error(
                f"OpenAI Length Finish Reason Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.LengthFinishReasonError"}}
            )
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            ...
            logger.error(
                f"OpenAI Content Filter Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.ContentFilterFinishReasonError"}}
            )
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("content_filter_issue")
            content = OPENAI_MESSAGES_CONFIG.get("content_filter_issue", OPENAI_MESSAGES_CONFIG.get("common_response"))
            ...
            logger.error(
                f"OpenAI Timeout Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.APITimeoutError"}}
            )
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("request_time_out")
            content = OPENAI_MESSAGES_CONFIG.get("request_time_out", OPENAI_MESSAGES_CONFIG.get("common_response"))
            ...
            logger.error(
                f"OpenAI Connection Error: {e}",
                extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.APIConnectionError"}}
            )
            thread_repo.initialization(thread_id, collection_name)
            thread_repo.add_message_openai("connection_error")
            content = OPENAI_MESSAGES_CONFIG.get("connection_error", OPENAI_MESSAGES_CONFIG.get("common_response"))
            ...
            try:
                ...
                if error_code not in OPENAI_MESSAGES_CONFIG:
                    logger.warning(
                        f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                        extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.Exception_Try"}})
                else:
                    logger.error(
                        f"🚨 Failed to stream run conversation: {error_code}, Message: {error_content}",
                        extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation.Exception_Try"}})
                ...
                logger.error(
                    f"🚨 Failed to stream run conversation: {e}",
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.stream_run_conversation_Exception_Except"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = OPENAI_MESSAGES_CONFIG.get("common_response")
                yield json.dumps({"status": status.HTTP_400_BAD_REQUEST, "message": DEV_MESSAGES_CONFIG.get("dev_message"), "data": content}), status.HTTP_400_BAD_REQUEST
        ...
        try:
            ...
            if cleaned_up:
                logger.info(
                    ...
                    extra={"tags": {"method": "OpenAICustomGPTStreamingDocChatServiceImg.cleanup"}}
                )
