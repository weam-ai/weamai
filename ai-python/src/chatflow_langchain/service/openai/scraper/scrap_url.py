...
from src.chatflow_langchain.service.openai.scraper.config import ScraperConfig
from src.chatflow_langchain.service.openai.scraper.config import GetLLMkey,DEFAULTMODEL
        ...
        try:
            ...
            if available_info is not None:
                ...
                process_chain = chain(
                    ...
                    generate_list_summaries.s(api_key=llm_apikey_decrypt_service.apikey,notification_data={"token": self.fcm_tokens,"user_data":user_data, "brain_id":str(brain_id)},companyRedis_id=self.companyRedis_id),
                    dump_to_db.s(parent_prompt_id=parent_prompt_ids[0],child_prompt_ids=child_prompt_ids,collection=collection)
                ).apply_async(
                    link=on_task_success.s(data={"token": self.fcm_tokens, "title": ScraperConfig.SUCCESS_TITLE.format(title=title,brain_title=brain_title), "body": ScraperConfig.SUCCESS_BODY.format(title=title,brain_title=brain_title),"user_data":user_data, "brain_id":str(brain_id)}),
                    link_error=on_task_failed.s(data={"token": self.fcm_tokens, "title": ScraperConfig.FAILURE_TITLE.format(title=title,brain_title=brain_title), "body": ScraperConfig.FAILURE_BODY.format(title=title,brain_title=brain_title), "user_data":user_data, "brain_id":str(brain_id)})
                )
