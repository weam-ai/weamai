...
from src.chatflow_langchain.service.gemini.custom_gpt.doc.rag_conversation import GeminiCustomGPTStreamingDocChatService
from src.chatflow_langchain.service.gemini.custom_gpt.simple_chat.system_conversation import GeminiCustomGPTStreamingSimpleChatService
from src.chatflow_langchain.service.gemini.custom_gpt.simple_chat.system_conversation_tool import GeminiCustomGPTStreamingSimpleChatServiceTool
from src.chatflow_langchain.service.gemini.custom_gpt.doc.rag_conversation_tool import GeminiCustomGPTStreamingDocChatServiceTool
...
class GeminiCustomGPTManager():
    def __init__(self):
        self.services = {
            "doc": GeminiCustomGPTStreamingDocChatServiceTool(),
            "simple": GeminiCustomGPTStreamingSimpleChatServiceTool(),
        }
        self.handler_map = {
            "simple": self.image_gen_custom_gpt_simple_handler,
            "doc": self.image_custom_gpt_doc_handler
        }
    ...
    async def custom_gpt_chat_handler(self, chat_input, **kwargs):
        ...
        if flag or chat_input.extra_file_ids:
            handler = self.handler_map.get('doc', self.image_gen_custom_gpt_simple_handler)
        else:
            handler = self.handler_map.get('simple', self.image_gen_custom_gpt_simple_handler)
