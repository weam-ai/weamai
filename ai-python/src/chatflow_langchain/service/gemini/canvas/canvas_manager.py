...
from src.chatflow_langchain.service.gemini.canvas.chat_prompt_factory import create_chat_prompt_canvas
from src.chatflow_langchain.service.gemini.canvas.chat_prompt_factory import chat_prompt_with_code_canvas,chat_prompt_with_customgpt,chat_prompt_with_doc_canvas,chat_prompt_with_customgpt_doc
...
from src.chatflow_langchain.service.gemini.canvas.config import CanvasConfig
...
class GeminiCanvasService():
    ...
    def extract_selected_text(self,start_index:int=0,end_index:int=None,user_query:str=None):
        ...
        try:
            ...
            logger.error(f"Failed to extract selected text: {e}",
                         extra={"tags": {"method": "GeminiCanvasService.extract_selected_text"}})
    ...
    def initialize_llm(self, api_key_id: str = None, companymodel: str = None):
        ...
        try:
            ...
            logger.error(f"Failed to initialize LLM: {e}",
                         extra={"tags": {"method": "GeminiCanvasService.initialize_llm"}})
    ...
    def initialize_memory(self):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "GeminiCanvasService.initialize_memory"}}
            )
    ...
    def initialize_repository(self, chat_session_id: str = None, chat_collection_name: str = None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "GeminiCanvasService.initialize_repository"}}
            )
    ...
    def initialize_thread_data(self, thread_id: str = None, collection_name: str = None,regenerated_flag:bool = False):
        ...
        try:
            ...
            logger.error(f"Failed to initialize thread data: {e}",
                         extra={"tags": {"method": "GeminiCanvasService.initialize_thread_data"}})
    ...
    def fetch_data_doc(self):
        try:
            ...
            try:
                ...
                thread_repo.add_message_gemini("embedding_model_required")
            ...
            logger.error(f"Error occurred during data fetching: {e}",
                         extra={"tags": {"method": "GeminiCanvasService.fetch_data_doc"}})
    ...
    def create_chain(self):
        ...
        try:
            ...
            logger.error(f"Failed to create chain: {e}",
                         extra={"tags": {"method": "GeminiCanvasService.create_chain"}})
    ...
    async def async_initialize_and_update(self, chat_session_id, chat_collection_name, new_thread_id, collection_name, cb, final_answer,msgCredit,is_paid_user):
        try:
            ...
            with gemini_sync_cost_handler(model_name=self.model_name) as summary_cb:
                self.memory.prune()
            # await self.api_usage_service.update_usage(provider=llm_apikey_decrypt_service.bot_data.get('code', 'GEMINI'),tokens_used= summary_cb.total_tokens, model=self.model_name, api_key=llm_apikey_decrypt_service.apikey,functionality=Functionality.CHAT,company_id=llm_apikey_decrypt_service.companyRedis_id)
            ...
            logger.info("Database successfully stored response",
                extra={"tags":{"method": "GeminiCanvasService.async_initialize_and_update"}})
        except Exception as e:
    ...
    async def run_chain(self, query:str=None,new_thread_id:str=None,chat_session_id: str = None, chat_collection_name: str = None,collection_name:str=None,delay_chunk:int=0,msgCredit:float=0,is_paid_user:bool=False):
        ...
        try:
            ...
            # await self.api_usage_service.update_usage(provider=llm_apikey_decrypt_service.bot_data.get('code', 'GEMINI'),tokens_used= cb.total_tokens, model=self.model_name, api_key=llm_apikey_decrypt_service.apikey,functionality=Functionality.CHAT,company_id=llm_apikey_decrypt_service.companyRedis_id)
        ...
        # Handle ResourceExhaustedError
            ...
            logger.error(
                ...
                extra={"tags": {"method": "GeminiCanvasService.run_chain.ResourceExhausted"}})
            ...
            logger.error(
                f"🚨 Google API Error: {error_content}",
                extra={"tags": {"method": "GeminiCanvasService.run_chain.GoogleAPICallError"}})
            thread_repo.initialization(self.thread_id, collection_name)
        ...
        # Handle GoogleAPIError
            ...
            logger.error(
                f"🚨 Google API Error: {error_content}",
                extra={"tags": {"method": "GeminiCanvasService.run_chain.GoogleAPIError"}})
            thread_repo.initialization(self.thread_id, collection_name)
            ...
            logger.error(
                f"🚨 Google API Error: {error_content}",
                extra={"tags": {"method": "GeminiCanvasService.run_chain.GoogleGenerativeAIError"}})
            thread_repo.initialization(self.thread_id, collection_name)
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        ...
                        extra={"tags": {"method": "GeminiCanvasService.Exception.extract_google_error_message"}})
                ...
                if not error_content:
                    try:
                        ...
                        logger.warning(
                            ...
                            extra={"tags": {"method": "GeminiCanvasService.Exception.extract_google_genai_error_message"}})
                ...
                logger.error(
                    ...
                    extra={"tags": {"method": "GeminiCanvasService.run_chain.Exception_Try"}})
                thread_repo.initialization(self.thread_id, collection_name)
                ...
                logger.error(
                    ...
                    extra={"tags": {"method": "GeminiCanvasService.run_chain.Exception_Except"}})
                thread_repo.initialization(self.thread_id, collection_name)
                thread_repo.add_message_gemini("common_response")
