...
from src.chatflow_langchain.service.gemini.title.chat_prompt_factory import create_chat_prompt_title
from src.chatflow_langchain.service.gemini.title.chat_prompt_factory import prompt_title_without_answer
from src.chatflow_langchain.service.gemini.title.custom_parser import TitleOutputParser
...
from src.chatflow_langchain.service.gemini.title.config import REFORMED_QUERY
...
class GeminiTitleGenerationService(AbstractTitleGeneration):
    ...
    def initialize_llm(self, api_key_id: str = None, companymodel: str = None):
        ...
        try:
            ...
            self.companyRedis=llm_apikey_decrypt_service.companyRedis_id
            self.model_name=GEMINIMODEL.GEMINI_2O_FLASH
            ...
            logger.error(f"Failed to initialize LLM: {e}",
                         extra={"tags": {"method": "GeminiTitleGenerationService.initialize_llm"}})
    ...
    def initialize_thread_data(self, thread_id: str = None, collection_name: str = None):
        ...
        try:
            ...
            logger.error(f"Failed to initialize thread data: {e}",
                         extra={"tags": {"method": "GeminiTitleGenerationService.initialize_thread_data"}})
    ...
    def create_prompt(self):
        ...
        try:
            ...
            logger.error(f"Failed to create prompt: {e}",
                         extra={"tags": {"method": "GeminiTitleGenerationService.create_prompt"}})
    ...
    def create_chain(self):
        ...
        try:
            ...
            logger.error(f"Failed to create chain: {e}",
                         extra={"tags": {"method": "GeminiTitleGenerationService.create_chain"}})
    ...
    def run_chain(self, chat_session_id: str = None, collection_name: str = None, collection_chatmember: str = None):
        ...
        # Handle ResourceExhaustedError
            ...
            logger.error(
                ...
                extra={"tags": {"method": "GeminiTitleGenerationService.run_chain.ResourceExhausted"}})
            thread_repo.initialization(self.thread_id, self.thread_collection)
            thread_repo.add_message_gemini("resource_exhausted_error")

            default_title = get_default_title("ResourceExhausted")
        ...
        # Handle ResourceExhaustedError
            ...
            default_title = get_default_title("GoogleAPICallError")
        ...
        # Handle ResourceExhaustedError
            ...
            default_title = get_default_title("GoogleAPIError")
        ...
        # Handle ResourceExhaustedError
            ...
            default_title = get_default_title("GoogleGenerativeAIError")
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(
                        ...
                        extra={"tags": {"method": "GeminiTitleGenerationService.Exception.extract_google_error_message"}})
                ...
                if not error_content:
                    try:
                        ...
                        logger.warning(
                            ...
                            extra={"tags": {"method": "GeminiTitleGenerationService.Exception.extract_google_genai_error_message"}})
                ...
                logger.error(
                    ...
                    extra={"tags": {"method": "GeminiTitleGenerationService.run_chain.Exception_Try"}})
                thread_repo.initialization(self.thread_id, self.thread_collection)
                ...
                logger.error(
                    ...
                    extra={"tags": {"method": "GeminiTitleGenerationService.run_chain.Exception_Except"}})
                thread_repo.initialization(self.thread_id, self.thread_collection)
                thread_repo.add_message_gemini("common_response")
        ...
        try:
            ...
            if cleaned_up:
                logger.info(
                    ...
                    extra={"tags": {"method": "GeminiTitleGenerationService.cleanup"}}
                )
