...
from src.celery_worker_hub.web_scraper.tasks.summary import generate_list_summaries_ant
...
from src.chatflow_langchain.service.gemini.scraper.config import ScraperConfig
from src.chatflow_langchain.service.gemini.config.model_config import DefaultSonnet35ModelRepository
import gc
from src.chatflow_langchain.service.gemini.image.utils import extract_google_error_message
...
class GeminiScrapUrlService():

    def initialize_llm(self, company_id: str = None, companymodel: str = None,llm_api_key_id:str=None):
        try:
            default_llm = DefaultSonnet35ModelRepository(company_id=company_id,companymodel=companymodel)
            llm_api_key_id = default_llm.get_default_model_key()
        ...
        try:
            ...
            if available_info is not None:
                ...
                process_chain = chain(
                    ...
                    generate_list_summaries_ant.s(api_key=llm_apikey_decrypt_service.apikey,notification_data={"token": self.fcm_tokens,"user_data":user_data, "brain_id":brain_id}),
            ...
            try:
                error_content,error_code = extract_google_error_message(str(e))
                if error_code not in GENAI_ERROR_MESSAGES_CONFIG:
                    logger.warning(
                        f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                        extra={"tags": {"method": "AnthropicScrapUrlService.scrape_website.Exception_Try"}})
                else:
                    logger.error(
                ...
                prompt_repo.add_message_openai(error_code)
                content = GENAI_ERROR_MESSAGES_CONFIG.get(error_code, GENAI_ERROR_MESSAGES_CONFIG.get("common_response"))
                raise HTTPException({"status": status.HTTP_417_EXPECTATION_FAILED,"message": error_content, "data": content})  
            except Exception as e:
                logger.error(
                    f"🚨 Failed to stream run conversation: {e}",
                    extra={"tags": {"method": "AnthropicScrapUrlService.scrape_website.Exception_Except"}})
                prompt_repo.initialization(self.prompt_id, self.collection_name)
                prompt_repo.add_message_openai(error_code)
                content = GENAI_ERROR_MESSAGES_CONFIG.get("common_response")
                raise HTTPException({"status": status.HTTP_400_BAD_REQUEST, "message": DEV_MESSAGES_CONFIG.get("dev_message"), "data": content})
