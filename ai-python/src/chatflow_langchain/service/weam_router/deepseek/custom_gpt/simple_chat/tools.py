...
from src.chatflow_langchain.service.weam_router.deepseek.tool_functions.config import ToolChatConfig
...
from src.chatflow_langchain.service.weam_router.deepseek.custom_gpt.simple_chat.utils import extract_error_message
from src.custom_lib.langchain.callbacks.openai.cost.cost_calc_handler import CostCalculator
...
@tool(description=DEEPSEEK_TOOL_DESCRIPTION.CUSTOM_GPT_SIMPLE_IMAGE)
async def image_generate(image_query:bool=False, model_name=None, image_quality=None, n=1, image_size=None, image_style=None,
                         openai_api_key=None, thread_id=None, thread_model=None, memory=None, chat_repository_history=None, original_query=None,api_key_id=None,regenerated_flag=False,user_system_prompt=None):
    ...
    try:
        ...
        cost=CostCalculator()
        async with dalle_callback_handler(llm_model=OPENAIMODEL.GPT_4_1_MINI, cost = cost,dalle_model=model_name,thread_id=thread_id,collection_name=thread_model,image_quality = image_quality,image_size=image_size,image_style=image_style) as asynchandler:
