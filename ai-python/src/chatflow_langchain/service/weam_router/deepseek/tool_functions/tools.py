...
from src.custom_lib.langchain.callbacks.openai.cost.cost_calc_handler import CostCalculator
from src.custom_lib.langchain.callbacks.weam_router.deep_seek.mongodb.context_manager import get_mongodb_callback_handler
from src.custom_lib.langchain.callbacks.weam_router.deep_seek.cost.context_manager import deepseek_async_callback
...
async def simple_chat_v2(query:bool=False, openai_api_key=None, temprature=None, model_name=None,
    ...
    try:
        ...
        llm = ChatOpenAI(temperature=temprature, api_key=openai_api_key, openai_api_base="https://openrouter.ai/api/v1",
        ...
        if image_url is not None and model_name in ToolChatConfig.VISION_MODELS:
            ...
            async with deepseek_async_callback(model_name=model_name,thread_id=thread_id,collection_name=thread_model) as cb,\
        ...
        if error_code not in WEAM_ROUTER_MESSAGES_CONFIG:
            logger.warning(
                ...
                extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.NotFoundError"}})
        else:
            logger.error(
        ...
        if error_code not in WEAM_ROUTER_MESSAGES_CONFIG:
            logger.warning(
                f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.RateLimitError"}})
        else:
            logger.error(
        ...
        if not error_code or error_code not in WEAM_ROUTER_MESSAGES_CONFIG:
            logger.warning(
                f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.APIStatusError"}})
            thread_repo.initialization(thread_id, thread_model)
        ...
        logger.error(
            ...
            extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.LengthFinishReasonError"}})
        thread_repo.initialization(thread_id, thread_model)
        ...
        logger.error(
            ...
            extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.ContentFilterFinishReasonError"}})
        thread_repo.initialization(thread_id, thread_model)
        thread_repo.add_message_weam_router("content_filter_issue")
        content = WEAM_ROUTER_MESSAGES_CONFIG.get("content_filter_issue", WEAM_ROUTER_MESSAGES_CONFIG.get("common_response"))
        yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
        ...
        logger.error(
            ...
            extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.APITimeoutError"}})
        thread_repo.initialization(thread_id, thread_model)
        ...
        logger.error(
            ...
            extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.APIConnectionError"}})
        thread_repo.initialization(thread_id, thread_model)
        ...
        try:
            ...
            if error_code not in WEAM_ROUTER_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.Exception_Try"}})
            else:
                logger.error(
            ...
            logger.error(
                ...
                extra={"tags": {"method": "SimpleToolFunction.simple_chat_v2.Exception_Except"}})
            thread_repo.initialization(thread_id, thread_model)
            thread_repo.add_message_weam_router("common_response")
            content = WEAM_ROUTER_MESSAGES_CONFIG.get("common_response")
...
@tool(description=DEEPSEEK_TOOL_DESCRIPTION.TOOLS_IMAGE_GENERATION)
