...
from src.chatflow_langchain.service.openai.title.chat_prompt_factory import create_chat_prompt_title
from src.chatflow_langchain.service.openai.title.chat_prompt_factory import prompt_title_without_answer
from src.chatflow_langchain.service.openai.title.custom_parser import TitleOutputParser
...
from src.chatflow_langchain.service.openai.title.config import REFORMED_QUERY
from src.chatflow_langchain.service.config.model_config_openai import OPENAIMODEL,DefaultOpenAIModelRepository
import gc
from src.chatflow_langchain.service.openai.title.utils import extract_error_message, get_default_title
...
class PerplexityTitleGenerationService(AbstractTitleGeneration):
    ...
    def initialize_llm(self, api_key_id: str = None, companymodel: str = None):
        ...
        try:
            ...
            self.company_id = llm_apikey_decrypt_service.company_id
            default_api_key = DefaultOpenAIModelRepository(company_id=self.company_id,companymodel=companymodel)
            ...
            self.model_name = OPENAIMODEL.GPT_4_1_MINI
            self.llm = ChatOpenAI(
            ...
            logger.error(f"Failed to initialize LLM: {e}",
                         extra={"tags": {"method": "Perplexity.initialize_llm"}})
    ...
    def initialize_thread_data(self, thread_id: str = None, collection_name: str = None):
        ...
        try:
            ...
            logger.error(f"Failed to initialize thread data: {e}",
                         extra={"tags": {"method": "Perplexity.initialize_thread_data"}})
    ...
    def create_prompt(self):
        ...
        try:
            ...
            logger.error(f"Failed to create prompt: {e}",
                         extra={"tags": {"method": "Perplexity.create_prompt"}})
    ...
    def create_chain(self):
        ...
        try:
            ...
            logger.error(f"Failed to create chain: {e}",
                         extra={"tags": {"method": "Perplexity.create_chain"}})
        ...
        try:
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    ...
                    extra={"tags": {"method": "Perplexity.run_chain.NotFoundError"}})
            else:
                logger.error(
            ...
            if error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "Perplexity.run_chain.RateLimitError"}})
            else:
                logger.error(
            ...
            if not error_code or error_code not in OPENAI_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "Perplexity.run_chain.APIStatusError"}})
                thread_repo.initialization(self.thread_id, self.thread_collection)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "Perplexity.run_chain.LengthFinishReasonError"}})
            thread_repo.initialization(self.thread_id, self.thread_collection)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "Perplexity.run_chain.ContentFilterFinishReasonError"}})
            thread_repo.initialization(self.thread_id, self.thread_collection)
            thread_repo.add_message_openai("content_filter_issue")
            ...
            logger.error(
                ...
                extra={"tags": {"method": "Perplexity.run_chain.APITimeoutError"}})
            thread_repo.initialization(self.thread_id, self.thread_collection)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "Perplexity.run_chain.APIConnectionError"}})
            thread_repo.initialization(self.thread_id, self.thread_collection)
            ...
            yield default_title
                ...
                try:
                    ...
                    if error_code not in OPENAI_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "Perplexity.run_chain.Exception_Except"}})
                    else:
                        logger.error(
        ...
        try:
            ...
            if cleaned_up:
                logger.info(
                    ...
                    extra={"tags": {"method": "Perplexity.cleanup"}}
                )
