...
from src.chatflow_langchain.service.openai.image.chat_prompt_factory import ImagePrompt
...
from src.chatflow_langchain.service.openai.image.config import ImageGenerateConfig
...
class OpenAIImageGenerationService(ImageGenerationAbstractRepository):
    """
    ...
    def initialize_llm_tool(self, api_key_id: str = None, companymodel: str = None,dalle_wrapper_size:str=None,dalle_wrapper_quality:str=None,dalle_wrapper_style:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "ImageGenerationService.initialize_llm_tool"}}
            )
    ...
    def initialize_repository(self, chat_session_id: str = None, collection_name: str = None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "ImageGenerationService.initialize_repository"}}
            )
    ...
    def initialize_memory(self):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "ImageGenerationService.initialize_memory"}}
            )
    ...
    def create_conversation(self, input_text:str=None,**kwargs):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "ImageGenerationService.create_conversation"}}
            )
    ...
    async def run_image_generation(self,thread_id: str, collection_name: str,**kwargs):
        ...
        # Handle NotFoundError
            ...
            if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    ...
                    extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.NotFoundError"}})
            else:
                logger.error(
        ...
        # Handle specific errors like RateLimitError.
            ...
            if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.RateLimitError"}})
            else:
                logger.error(
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.APIStatusError"}})
                thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.AuthenticationError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.PermissionDeniedError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.APIConnectionError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.APITimeoutError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.APIError"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.AnthropicError"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": error_content, "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.Exception_Try"}})
                    else:
                        logger.error(
                    ...
                    logger.error(
                        ...
                        extra={"tags": {"method": "OpenAIImageGenerationService.run_image_generation.Exception_Except"}})
                    thread_repo.initialization(thread_id, collection_name)
                    thread_repo.add_message_openai("common_response")
                    content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
        ...
        try:
            ...
            if cleaned_up:
                logger.info(
                    ...
                    extra={"tags": {"method": "ImageGenerationService.cleanup"}}
                )
