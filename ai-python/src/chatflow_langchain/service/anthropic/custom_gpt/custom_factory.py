...
from src.chatflow_langchain.service.anthropic.custom_gpt.doc.rag_conversation import AnthropicCustomGPTStreamingDocChatService
from src.chatflow_langchain.service.anthropic.custom_gpt.simple_chat.system_conversation import AnthropicCustomGPTStreamingSimpleChatService
from src.chatflow_langchain.repositories.custom_gpt_repository import CustomGPTRepository
from src.chatflow_langchain.service.anthropic.custom_gpt.simple_chat.system_conversation_tool import AnthropicCustomGPTStreamingSimpleChatServiceTool
from src.chatflow_langchain.service.anthropic.custom_gpt.doc.rag_conversation_tool import AnthropicCustomGPTStreamingDocChatServiceTool

custom_gpt_repo = CustomGPTRepository()

class AnthropicCustomGPTManager():
    def __init__(self):
        self.services = {
            "doc": AnthropicCustomGPTStreamingDocChatService(),
            "simple": AnthropicCustomGPTStreamingSimpleChatService(),
            "simple_img_gen": AnthropicCustomGPTStreamingSimpleChatServiceTool(),
            "doc_img_gen": AnthropicCustomGPTStreamingDocChatServiceTool()
        }
        self.handler_map = {
            "doc": self.custom_gpt_doc_handler,
            "simple": self.custom_gpt_simple_handler,
            "simple_img_gen": self.image_gen_custom_gpt_simple_handler,
            "doc_img_gen": self.image_custom_gpt_doc_handler
        }
    ...
    async def image_gen_custom_gpt_simple_handler(self,chat_input,**kwargs): 
        customgpt_simple_chat=self.services['simple_img_gen']
        query = chat_input.query
        customgpt_simple_chat.Initilization_custom_gpt(
            custom_gpt_id=chat_input.custom_gpt_id,
            customgptmodel=chat_input.customgptmodel
        ...
        customgpt_simple_chat.prompt_attach(additional_prompt_id=chat_input.prompt_id, collection_name=chat_input.promptmodel)
        customgpt_simple_chat.create_conversation(input_text=query,image_url=chat_input.image_url,regenerated_flag=chat_input.isregenerated)
        response_generator = customgpt_simple_chat.tool_calls_run(chat_input.thread_id, chat_input.threadmodel, isMedia=chat_input.isMedia,delay_chunk=chat_input.delay_chunk,regenerated_flag=chat_input.isregenerated,msgCredit=chat_input.msgCredit,is_paid_user=chat_input.is_paid_user)
        return response_generator
    

    async def image_custom_gpt_doc_handler(self,chat_input,**kwargs): 
        
        customgpt_doc_chat=self.services['doc_img_gen']
        query = chat_input.query
        customgpt_doc_chat.Initilization_custom_gpt(
            custom_gpt_id=chat_input.custom_gpt_id,
            customgptmodel=chat_input.customgptmodel
        ...
        customgpt_doc_chat.prompt_attach(additional_prompt_id=chat_input.prompt_id, collection_name=chat_input.promptmodel)
        customgpt_doc_chat.create_conversation(input_text=query, image_url=chat_input.image_url, regenerated_flag=chat_input.isregenerated)
        response_generator = customgpt_doc_chat.tool_calls_run(chat_input.thread_id, chat_input.threadmodel, isMedia=chat_input.isMedia,delay_chunk=chat_input.delay_chunk,regenerated_flag=chat_input.isregenerated,msgCredit=chat_input.msgCredit,is_paid_user=chat_input.is_paid_user)

        return response_generator
    async def custom_gpt_chat_handler(self, chat_input, **kwargs):
        # Determine the handler based on the prefix of custom_gpt_id
     
        flag=custom_gpt_repo.get_doc_flag()
        img_flag=custom_gpt_repo.get_img_flag()
        if flag or chat_input.extra_file_ids:  
                handler = self.handler_map.get('doc_img_gen', self.image_gen_custom_gpt_simple_handler)
        else:
                handler = self.handler_map.get('simple_img_gen', self.image_gen_custom_gpt_simple_handler)
