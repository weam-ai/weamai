...
class AnthropicCustomGPTStreamingSimpleChatService(AbstractConversationService):
    ...
    async def stream_run_conversation(self, thread_id: str, collection_name: str,regenerated_flag:bool=False,msgCredit:float=0,is_paid_user:bool=False,**kwargs) -> AsyncGenerator[str, None]:
        ...
        # Handle NotFoundError
            ...
            if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    ...
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.NotFoundError"}})
            else:
                logger.error(
        ...
        # Handle specific errors like RateLimitError.
            ...
            if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.RateLimitError"}})
            else:
                logger.error(
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.APIStatusError"}})
                thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.AuthenticationError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.PermissionDeniedError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.APIConnectionError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.APITimeoutError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.APIError"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.AnthropicError"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": error_content, "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.Exception_Try"}})
                    else:
                        logger.error(
                    ...
                    logger.error(
                        ...
                        extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatService.stream_run_conversation.Exception_Except"}})
                    thread_repo.initialization(thread_id, collection_name)
                    thread_repo.add_message_openai("common_response")
                    content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
