...
from src.chatflow_langchain.service.anthropic.custom_gpt.simple_chat.chat_prompt_factory import UserCustomGPTPrompt
...
from src.custom_lib.langchain.callbacks.anthropic.cost.context_manager import anthropic_async_callback,anthropic_sync_callback
...
from src.chatflow_langchain.service.anthropic.custom_gpt.config import CustomGptChatConfig
...
from src.chatflow_langchain.service.anthropic.custom_gpt.simple_chat.utils import extract_anthropic_error_message
...
from src.chatflow_langchain.service.anthropic.custom_gpt.config import ToolChatConfig
...
class AnthropicCustomGPTStreamingSimpleChatServiceTool(AbstractConversationService):
    def Initilization_custom_gpt(self,custom_gpt_id:str=None,customgptmodel:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.Initilization_custom_gpt"}}
            )
    ...
    def initialize_repository(self, chat_session_id: str = None, collection_name: str = None,regenerated_flag:bool=False,thread_id:str=None):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.initialize_repository"}}
            )
    ...
    def initialize_llm(self, company_id:str=None,companymodel: str = None,**kwargs):
        ...
        try:
            ...
            if self.api_key_id is None:
                ...
                self.api_key_id=default_llm.get_default_model_key()
            ...
            self.llm_non_stream = ChatAnthropic(
                model_name=ANTHROPICMODEL.DEFAULT_TOOL_MODEL,
                temperature=llm_apikey_decrypt_service.extra_config.get('temperature'),
                api_key=llm_apikey_decrypt_service.decrypt(),
                streaming=False,
                verbose=False,
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.initialize_llm"}}
            )
    ...
    def initialize_memory(self):
        ...
        try:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.initialize_memory"}}
            )
    ...
    def prompt_attach(self, additional_prompt_id: str = None, collection_name: str = None):
        ...
        try:
            if additional_prompt_id:
                ...
                if resource_key is not None and resource_value is not None:
                    ...
                    logger.info("Successfully attached additional prompt", extra={
                        "tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.prompt_attach"},
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.prompt_attach"}}
            )
    ...
    def create_conversation(self,input_text:str,**kwargs):
        ...
        try:
            ...
            if self.image_url is not None and self.model_name in ANTHROPICMODEL.VISION_MODELS:
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.create_conversation"}}
            )
    ...
    async def tool_calls_run(self, thread_id: str, collection_name: str,regenerated_flag=False, msgCredit=0,is_paid_user=False,**kwargs) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # await self.api_usage_service.update_usage_anthropic(provider=llm_apikey_decrypt_service.bot_data.get('code', 'ANTHROPIC'),tokens_used= cb, model=self.model_name, api_key=llm_apikey_decrypt_service.apikey,functionality=Functionality.CHAT,company_id=self.companyRedis_id)
            for tool_call in ai_msg.tool_calls:
                ...
                "tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run"}
            }) 
        ...
        # Handle NotFoundError
            ...
            if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    ...
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.NotFoundError"}})
            else:
                logger.error(
        ...
        # Handle specific errors like RateLimitError.
            ...
            if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.RateLimitError"}})
            else:
                logger.error(
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.APIStatusError"}})
                thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.AuthenticationError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.PermissionDeniedError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.APIConnectionError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            logger.error(
                ...
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.APITimeoutError"}})
            thread_repo.initialization(thread_id, collection_name)
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.APIError"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.AnthropicError"}})
                thread_repo.initialization(thread_id, collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": error_content, "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.Exception_Try"}})
                    else:
                        logger.error(
                    ...
                    logger.error(
                        ...
                        extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.tool_calls_run.Exception_Except"}})
                    thread_repo.initialization(thread_id, collection_name)
                    thread_repo.add_message_openai("common_response")
                    content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
    ...
    async def wrapper_website_analysis(self,implicit_reference_urls:list[str]=[],**kwargs):
        pass
    async def website_analysis(self):
        try:
            logger.info(f"Tool 'website_analysis' called", extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.website_analysis"}})
            ...
            async with  \
                    anthropic_async_callback(self.model_name, cost=cost_callback, thread_id=thread_id, collection_name=thread_model,encrypted_key=self.encrypted_key,companyRedis_id=self.companyRedis_id) as cb, \
                    get_mongodb_callback_handler(thread_id=thread_id, chat_history=self.chat_repository_history, memory=self.memory,collection_name=self.collection_name,regenerated_flag=self.regenerated_flag,model_name=llm_apikey_decrypt_service.model_name,msgCredit=self.msgCredit,is_paid_user=self.is_paid_user,encrypted_key=self.encrypted_key,companyRedis_id=self.companyRedis_id) as mongo_handler:
                        # Stream the response
            ...
            if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.NotFoundError"}})
            else:
                logger.error(
                    f"🚨 Model Not Found Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.NotFoundError"}})
            thread_repo.initialization(thread_id, self.collection_name)
            thread_repo.add_message_openai(error_code)
        ...
        # Handle specific errors like RateLimitError.
            ...
            if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.RateLimitError"}})
            else:
                logger.error(
                    f"🚨 Rate Limit Error: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.RateLimitError"}})
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.APIStatusError"}})
                thread_repo.initialization(thread_id, self.collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            logger.error(
                f"Anthropic Authentication Error: {e}",
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.AuthenticationError"}})
            thread_repo.initialization(thread_id, self.collection_name)
            thread_repo.add_message_openai("authentication_error")
            content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("authentication_error", ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"Anthropic PermissionDenied Error: {e}",
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.PermissionDeniedError"}})
            thread_repo.initialization(thread_id, self.collection_name)
            thread_repo.add_message_openai("permission_error")
            content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("permission_error", ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"Anthropic APIConnection Error: {e}",
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.APIConnectionError"}})
            thread_repo.initialization(thread_id, self.collection_name)
            thread_repo.add_message_openai("api_connection_error")
            content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("api_connection_error", ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            logger.error(
                f"Anthropic APITimeout Error: {e}",
                extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.APITimeoutError"}})
            thread_repo.initialization(thread_id, self.collection_name)
            thread_repo.add_message_openai("api_timeout_error")
            content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("api_timeout_error", ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response"))
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": e, "data": content}), status.HTTP_417_EXPECTATION_FAILED
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.APIError"}})
                thread_repo.initialization(thread_id, self.collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            if not error_code or error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                logger.warning(
                    f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.AnthropicError"}})
                thread_repo.initialization(thread_id, self.collection_name)
                thread_repo.add_message_openai("common_response")
                content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                error_content = DEV_MESSAGES_CONFIG.get("unknown_message")
            ...
            yield json.dumps({"status": status.HTTP_417_EXPECTATION_FAILED, "message": error_content, "data": content}), status.HTTP_417_EXPECTATION_FAILED
                ...
                try:
                    ...
                    if error_code not in ANTHROPIC_ERROR_MESSAGES_CONFIG:
                        logger.warning(
                            f"👁️ NEW ERROR CODE FOUND: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.Exception_Try"}})
                    else:
                        logger.error(
                            f"🚨 Failed to stream run conversation: {error_code}, Message: {error_content}",
                            extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.Exception_Try"}})
                    ...
                    logger.error(
                        f"🚨 Failed to stream run conversation: {e}",
                        extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.stream_run_conversation.Exception_Except"}})
                    thread_repo.initialization(thread_id, self.collection_name)
                    thread_repo.add_message_openai("common_response")
                    content = ANTHROPIC_ERROR_MESSAGES_CONFIG.get("common_response")
                    yield json.dumps({"status": status.HTTP_400_BAD_REQUEST, "message": DEV_MESSAGES_CONFIG.get("dev_message"), "data": content}), status.HTTP_400_BAD_REQUEST
    ...
    async def simple_run_conversation_v1(self) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            async with async_streaming_handler() as async_handler, \
                    anthropic_async_callback(llm_apikey_decrypt_service.model_name, cost=cost, thread_id=thread_id, collection_name=collection_name,encrypted_key=self.encrypted_key,companyRedis_id=self.companyRedis_id,**self.kwargs) as cb, \
        ...
        try:
            ...
            if cleaned_up:
                logger.info(
                    ...
                    extra={"tags": {"method": "AnthropicCustomGPTStreamingSimpleChatServiceTool.cleanup"}}
                )
