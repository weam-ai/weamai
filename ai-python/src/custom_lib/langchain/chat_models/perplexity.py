...
class ChatPerplexity(BaseChatModel):
    ...
    **kwargs: Any,
        ...
        # Ensure 'stream' is only set once
        params["stream"] = True
        ...
        for chunk in stream_resp:
            ...
            usage = chunk.get("usage", {})  # Get token usage if available

            # Convert usage values to strings if necessary
            usage_serializable = {
            ...
            generation_info = {
                ...
                "usage": usage_serializable,  # Include token usage as serializable
            } if finish_reason is not None else {"citations": citations, "usage": usage_serializable}
            ...
            if run_manager:
                run_manager.on_llm_new_token(chat_chunk.message.text, chunk=chat_chunk)
            yield chat_chunk
    async def _astream(
    self,
    messages: List[BaseMessage],
    ...
    **kwargs: Any,
        ...
        # Use a synchronous method for stream_resp
        stream_resp = self.client.chat.completions.create(
            messages=message_dicts, **params
        )

        # Convert the synchronous stream_resp to an async generator
        for chunk in stream_resp:
            # Wrap processing in an async function for compatibility
            ...
            usage = chunk.get("usage", {})  # Get token usage if available
            return_questions = chunk.get("related_questions", [])
            ...
            generation_info = {
                ...
                "images":images, # Include token usage as serializable
                "related_questions":return_questions
            } if finish_reason is not None else {"citations": citations,"images":images}
            ...
            if run_manager:
                await run_manager.on_llm_new_token(chat_chunk.message.text, chunk=chat_chunk)
            yield chat_chunk
